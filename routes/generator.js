...
router.get(
  "/all/:start/:end",
  [check("start").not().isEmpty(), check("end").not().isEmpty()],
  validateInput,
...
router.get("/staking/weekly", validateInput, async function (req, res, next) {
  try {
    const staking = await axios.get("https://lto.tools/generators-weekly/json");

    res.status(200).end(staking.data);
  } catch (err) {
    next(err);
  }
});
...
router.get("/staking/monthly", validateInput, async function (req, res, next) {
  try {
    const staking = await axios.get(
      "https://lto.tools/generators-monthly/json"
    );
