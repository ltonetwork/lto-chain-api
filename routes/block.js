...
router.get('/last', async function (req, res, next) {
  try {
    ...
    // .innerJoin('transactions', 'blocks.index', 'transactions.block')
      .select(db.raw('blocks.index, blocks.reference, blocks.generator, blocks.signature, blocks.size, blocks.count, blocks.fee, blocks.version, from_unixtime(blocks.timestamp, "%Y-%m-%d"), blocks.verified, consensus.signature, consensus.target'))
...
router.get('/unverified', async function (req, res, next) {
  try {
    const getBlocks = await db('blocks')
      .leftJoin('consensus', 'blocks.index', 'consensus.index')
    // .innerJoin('transactions', 'blocks.index', 'transactions.block')
      .select(db.raw('blocks.index, blocks.reference, blocks.generator, blocks.signature, blocks.size, blocks.count, blocks.fee, blocks.version, from_unixtime(from_unixtime(blocks.timestamp, "%Y-%m-%d"), "%Y-%m-%d"), blocks.verified, consensus.signature, consensus.target'))
